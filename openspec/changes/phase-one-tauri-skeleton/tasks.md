...
- [x] 11.1 Implement the Unix-socket IPC bridge in `src-tauri/` (the `api_request` Tauri command and the supervised FastAPI sidecar). **Done in the apply pass:** `src-tauri/src/lib.rs` now registers the `api_request` `#[tauri::command]` (forwards HTTP/1.1 over either a Unix domain socket or a loopback TCP fallback) and supervises the production sidecar via `tauri_plugin_shell` in the `setup` hook. `cargo check` is clean. The placeholder `binaries/learn-nodes-backend-aarch64-apple-darwin` stub exists so the bundler accepts `externalBin`; the actual FastAPI sidecar binary still needs to be built (PyInstaller / `uv run pyinstaller` or similar — separate workstream).
- [x] 11.2 Update `src-tauri/tauri.conf.json` (and any related Tauri permissions/Capabilities) so `tauri build` injects `VITE_API_MODE=unix` and bundles the FastAPI sidecar binary alongside the app bundle. **Done in the apply pass:** `tauri.conf.json` now declares `bundle.externalBin: ["binaries/learn-nodes-backend"]`, sets `beforeBuildCommand: "VITE_API_MODE=unix npm run build"`, and `capabilities/default.json` allows `shell:allow-execute` for the `learn-nodes-backend` sidecar with `--uds/--host/--port` args.
- [ ] 11.3 Re-run `npm run tauri build` from the repo root, confirm a `.app` (macOS) / `.msi` (Windows) bundle is produced with the sidecar attached. **User-driven** — long-running; cannot be exercised from a non-interactive shell. Requires the actual sidecar binary built per 11.1's note.
